...
// string getUserName(json &js) { return js["name"]; }

ChatService::ChatService()
{
    // 对各类消息处理方法的注册
    _msgHandlerMap.insert({LOGIN_MSG, bind(&ChatService::loginHandler, this, _1, _2, _3)});
    _msgHandlerMap.insert({REGISTER_MSG, bind(&ChatService::registerHandler, this, _1, _2, _3)});
    _msgHandlerMap.insert({ONE_CHAT_MSG, bind(&ChatService::oneChatHandler, this, _1, _2, _3)});
    _msgHandlerMap.insert({ADD_FRIEND_MSG, bind(&ChatService::addFriendHandler, this, _1, _2, _3)});
    // 群组业务管理相关事件处理回调注册
...
void ChatService::loginHandler(const TcpConnectionPtr &conn, json &js, Timestamp time)
{
    ...
    if (user.getId() == id && user.getPassword() == password)
    {
        ...
        else
        {
            ...
            { // 大括号代表作用域，临界区代码段，左括号加锁
                // lock_guard创建时加锁，析构时解锁。
                // 为了防止在线程使用mutex加锁后异常退出导致死锁的问题，建议使用lock_guard代替mutex。
                lock_guard<mutex> lock(_connMutex);
                _userConnMap.insert({id, conn}); // 记录连接信息，进行通信
            }                                    // 右括号解锁
